...
/** Severity → dot ink for triage rows. One map, so a "warning" can never
 *  be amber on Home and red on Insights. */
export const toneDot: Record<"danger" | "warning" | "neutral", string> = {
...
/** One triage row: severity dot, what and why, one action. `compact`
 *  (Home's card) truncates to a single line each; `spacious` (Insights)
 *  lets the reason wrap at full reading size. */
export function AttentionRow({
  ...
  const action = item.action.to ? (
    <Button variant="secondary" size="sm" className="shrink-0" asChild>
      <Link to={item.action.to}>{item.action.label}</Link>
    </Button>
  ) : (
    <Button variant="secondary" size="sm" className="shrink-0" onClick={item.action.onClick}>
      {item.action.label}
    </Button>
  );
  return (
    <li
      className={cn(
        "flex gap-3",
        compact ? "items-center py-2.5" : "items-start py-3 first:pt-1",
      )}
    >
      <span
        aria-hidden
        className={cn("h-2 w-2 shrink-0 rounded-pill", !compact && "mt-1.5", toneDot[item.tone])}
      />
      <div className="min-w-0 flex-1">
        <p
          className={cn(
            "font-display text-sm font-semibold text-text-primary",
            compact && "truncate leading-5",
          )}
        >
          {item.title}
        </p>
        <p
          className={cn(
            compact
              ...
              : "mt-0.5 text-sm leading-5 text-text-secondary",
          )}
        >
          {item.reason}
        </p>
...
/** The queue those rows stack into — hairline-divided, card-body flush. */
export function AttentionList({
  ...
  return (
    <ul className={cn("divide-y divide-border-default", className)}>
      {items.map((item) => (
        <AttentionRow key={item.id ?? item.title} item={item} variant={variant} />
      ))}
    </ul>
  );
}
...
export function ReadyDecisionRow({
  ...
  layout?: "card" | "row";
  /** The ready-state verdict ("Results ready", "Target reached"). */
  eyebrow: string;
